...
bool MPU9250::probe() {
  ...
  WriteReg(
      ...
      AK8963_I2C_ADDR |
          READ_FLAG); // Set the I2C slave addres of AK8963 and set for read.
  WriteReg(MPUREG_I2C_SLV0_REG, AK8963_WIA); // I2C slave 0 register address
                                             // from where to begin data transfer
...
bool MPU9250::initialize() {
  ...
  uint8_t MPU_Init_Data[MPU_InitRegNum][2] = {
      ...
      {0x0D,
       MPUREG_I2C_MST_CTRL}, //  I2C configuration multi-master  IIC 400KHz
      ...
      {AK8963_CNTL1, MPUREG_I2C_SLV0_REG}, // I2C slave 0 register address from
                                           // where to begin data transfer
      {0x12,
       MPUREG_I2C_SLV0_DO}, // Register value to continuous measurement in 16bit
      {0x81, MPUREG_I2C_SLV0_CTRL} // Enable I2C and set 1 byte
  ...
  for (i = 0; i < MPU_InitRegNum; i++) {
    ...
    usleep(
        100000); // I2C must slow down the write speed, otherwise it won't work
  }
...
void MPU9250::update() {
  ...
  WriteReg(
      ...
      AK8963_I2C_ADDR |
          READ_FLAG); // Set the I2C slave addres of AK8963 and set for read.
  WriteReg(MPUREG_I2C_SLV0_REG, AK8963_HXL); // I2C slave 0 register address
                                             // from where to begin data transfer
